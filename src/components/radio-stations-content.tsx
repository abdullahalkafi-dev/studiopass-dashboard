    ...
    <div className="space-y-6">
      <div className="flex items-start justify-between">
        ...
        <div className="flex items-center gap-2.5">
          ...
          <Link href="/station-management/radio/create" className="flex items-center gap-2 px-4 py-2.5 bg-[#02B2FF] text-white rounded-lg text-sm font-semibold hover:bg-[#00A0E8] transition-colors shadow-sm">
        ...
        <div className="flex items-center gap-3">
          ...
          {isSuperAdmin && (
            <FilterSelect value={countryFilter} onChange={(v) => { setCountryFilter(v); setPg(1); }}
              options={countries.map((c: any) => ({ value: c.id, label: `${c.name} (${c.code})` }))} placeholder="All Countries" className="w-48" />
          )}
          {isSuperAdmin && (
            <FilterSelect value={partnerFilter} onChange={(v) => { setPartnerFilter(v); setPg(1); }}
              options={partners.map((p: any) => ({ value: p.id, label: p.name }))} placeholder="All Partners" className="w-48" />
          )}
          <FilterSelect value={statusFilter} onChange={(v) => { setStatusFilter(v); setPg(1); }}
            options={[{ value: "true", label: "Active" }, { value: "false", label: "Inactive" }]} placeholder="All Status" className="w-40" />
        </div>
      </div>
