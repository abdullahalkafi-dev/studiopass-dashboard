...
import {
  Monitor, Download, Plus, Search, Eye, Edit2, Power, PowerOff,
...
export default function TvStationsContent() {
  ...
  const { data, isLoading } = useGetStationsQuery({
    page: pg, limit: PER_PAGE, category: "tv",
  ...
  const handleToggleStatus = async (id: string, currentActive: boolean) => {
    if (currentActive) {
      await deactivateStation(id).unwrap().then(() => toast.success("Station deactivated")).catch(() => toast.error("Failed"));
    } else {
      await reactivateStation(id).unwrap().then(() => toast.success("Station reactivated")).catch(() => toast.error("Failed"));
    }
  };
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-[#EFF8FF] flex items-center justify-center text-[#02B2FF] shrink-0"><Monitor size={18} /></div>
          <div>
            <h1 className="text-xl font-bold text-foreground">TV Stations</h1>
            <p className="text-sm text-muted-foreground mt-0.5">Manage all TV stations on the platform.</p>
          </div>
        </div>
        <div className="flex items-center gap-2.5 w-full sm:w-auto">
          ...
          <Link href="/station-management/tv/create" className="flex-1 sm:flex-initial justify-center flex items-center gap-2 px-4 py-2.5 bg-[#02B2FF] text-white rounded-lg text-sm font-semibold hover:bg-[#00A0E8] transition-colors shadow-sm">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        <KpiCard label="Total Stations" value={String(total)} icon={<Monitor size={16} className="text-[#02B2FF]" />} iconBg="bg-[#EFF8FF]" />
        ...
        <KpiCard label="Countries" value={String(new Set(rows.map((r: any) => typeof r.country === "object" ? r.country?.id : r.country)).size)} icon={<Monitor size={16} className="text-violet-500" />} iconBg="bg-violet-50" />
      </div>
      ...
      <div className="bg-card rounded-xl border border-border shadow-sm p-3.5 sm:p-4">
        <div className="flex flex-col sm:flex-row sm:items-center gap-3">
          ...
          {isSuperAdmin && <FilterSelect value={countryFilter} onChange={(v) => { setCountryFilter(v); setPg(1); }} options={countries.map((c: any) => ({ value: c.id, label: `${c.name} (${c.code})` }))} placeholder="All Countries" className="w-full sm:w-44" />}
          {isSuperAdmin && <FilterSelect value={partnerFilter} onChange={(v) => { setPartnerFilter(v); setPg(1); }} options={partners.map((p: any) => ({ value: p.id, label: p.name }))} placeholder="All Partners" className="w-full sm:w-44" />}
          <FilterSelect value={statusFilter} onChange={(v) => { setStatusFilter(v); setPg(1); }} options={[{ value: "true", label: "Active" }, { value: "false", label: "Inactive" }]} placeholder="All Status" className="w-full sm:w-40" />
        </div>
      </div>
      ...
      <div className="bg-card rounded-xl border border-border shadow-sm overflow-hidden">
        ...
        <div className="overflow-x-auto rounded-xl border border-border">
          <table className="w-full min-w-[700px] text-sm">
            ...
            <tbody>
              {rows.length === 0 ? (
                <tr><td colSpan={isSuperAdmin ? 7 : 5} className="px-5 py-12 text-center text-sm text-muted-foreground">No TV stations found.</td></tr>
              ) : rows.map((row: any) => (
