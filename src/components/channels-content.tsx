    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2.5 w-full sm:w-auto">
          ...
          <Link href="/station-management/channels/create" className="flex-1 sm:flex-initial justify-center flex items-center gap-2 px-4 py-2.5 bg-[#02B2FF] text-white rounded-lg text-sm font-semibold hover:bg-[#00A0E8] transition-colors shadow-sm">
      ...
      <div className="bg-card rounded-xl border border-border shadow-sm overflow-hidden">
        ...
        <div className="overflow-x-auto rounded-xl border border-border">
          <table className="w-full min-w-[700px] text-sm">
            ...
            <tbody>
              {rows.length === 0 ? (
                <tr><td colSpan={isSuperAdmin ? 8 : 6} className="px-5 py-12 text-center text-sm text-muted-foreground">No channels found.</td></tr>
              ) : rows.map((row: any) => (
                <tr key={row.id} className="border-b border-border last:border-0 hover:bg-muted/20 transition-colors">
                  ...
                  <td className="px-5 py-3.5">
                    <StatusBadge
                      label={row.channelType === "challenges" ? "Challenges"
                        : row.channelType === "polls" ? "Polls / Vote"
                        : row.channelType === "message_chat" ? "Message / Chat"
                        : "—"}
                      variant={sv(row.channelType ? "Active" : "Inactive")}
                    />
                  </td>
