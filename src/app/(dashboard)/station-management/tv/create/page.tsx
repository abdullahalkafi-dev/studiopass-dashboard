...
export default function CreateTvStationPage() {
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      toast.success("TV station and admin created successfully");
      router.push("/station-management/tv");
    } catch (err: any) {
      toast.error(err?.data?.message || "Failed to create TV station");
    }
  };

  return (
    <div className="max-w-2xl space-y-6">
      <Link href="/station-management/tv" className="inline-flex items-center gap-1.5 text-xs font-semibold text-muted-foreground hover:text-[#02B2FF] transition-colors">
        <ArrowLeft size={13} /> Back to TV Stations
      </Link>

      <div>
        <h1 className="text-xl font-bold text-foreground">Add TV Station</h1>
        <p className="text-sm text-muted-foreground mt-1">Register a new TV station on the platform</p>
      </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <Input placeholder="e.g. Citizen TV" {...register("name")} />
              {errors.name && <p className="text-xs text-red-500 mt-1">{errors.name.message}</p>}
            </div>
            <div>
              <label className="block text-xs font-semibold text-foreground mb-1.5">Station Type</label>
              <input type="text" value="TV" disabled className="w-full px-3 py-2.5 text-sm rounded-lg border border-border bg-muted text-muted-foreground cursor-not-allowed" />
            </div>
          </div>
          ...
          <div>
            ...
            <Input placeholder="e.g. CIT-TV-KE" {...register("stationCode")} />
          ...
          <div className="border-t border-border pt-5">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <Input placeholder="e.g. citizen_tv" {...register("adminUsername")} />
          ...
          <div className="flex items-center gap-3 pt-2">
            <Button type="submit" disabled={isLoading} className="bg-[#02B2FF] hover:bg-[#0190D0] text-white">
              ...
              {isLoading ? "Creating..." : "Create TV Station"}
            </Button>
            <Link href="/station-management/tv">
