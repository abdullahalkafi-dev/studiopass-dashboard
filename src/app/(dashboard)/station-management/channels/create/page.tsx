...
export default function CreateChannelPage() {
    ...
    try {
      ...
      if (result.station?.id) {
        try {
          ...
          toast.warning("Channel created but logo upload failed. You can upload it later.");
        }
        if (coverFile) {
          try {
      ...
      toast.success("Channel and admin created successfully");
      router.push("/station-management/channels");
    } catch (err: any) {
  ...
  return (
    <div className="max-w-2xl space-y-6">
      <Link href="/station-management/channels" className="inline-flex items-center gap-1.5 text-xs font-semibold text-muted-foreground hover:text-[#02B2FF] transition-colors">
        <ArrowLeft size={13} /> Back to Channels
      </Link>

      <div>
        <h1 className="text-xl font-bold text-foreground">Add Channel</h1>
        <p className="text-sm text-muted-foreground mt-1">Register a new channel on the platform</p>
      </div>
      ...
      <Card className="p-6">
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-semibold text-foreground mb-1.5">Channel Name<span className="text-red-500 ml-0.5">*</span></label>
              <Input placeholder="e.g. Capital Sports" {...register("name")} />
          ...
          <div>
            <label className="block text-xs font-semibold text-foreground mb-1.5">Channel Code<span className="text-red-500 ml-0.5">*</span></label>
            <Input placeholder="e.g. CAP-SP-KE" {...register("stationCode")} />
            {errors.stationCode && <p className="text-xs text-red-500 mt-1">{errors.stationCode.message}</p>}
          </div>

          {/* Logo — Required */}
          <div>
            <label className="block text-xs font-semibold text-foreground mb-1.5">Channel Logo<span className="text-red-500 ml-0.5">*</span></label>
            {logoPreview ? (
              <div className="relative w-32 h-32 border-2 border-border rounded-xl overflow-hidden">
                <img src={logoPreview} alt="Logo preview" className="w-full h-full object-cover" />
                <button type="button" onClick={removeLogo} className="absolute top-1 right-1 w-6 h-6 bg-red-500 text-white rounded-full flex items-center justify-center hover:bg-red-600 transition-colors">
                  <X size={12} />
                </button>
              </div>
            ) : (
          ...
          <div className="border-t border-border pt-5">
            <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide mb-4">Channel Admin Account</p>
            <div className="grid grid-cols-2 gap-4">
              <div>
              ...
              <div>
                ...
                <Input placeholder="e.g. capital_sports" {...register("adminUsername")} />
          ...
          <div className="flex items-center gap-3 pt-2">
            ...
            <Link href="/station-management/channels">
