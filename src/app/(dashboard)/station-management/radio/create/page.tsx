...
export default function CreateRadioStationPage() {
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      toast.error(err?.data?.message || "Failed to create radio station");
    }
  };
  ...
  return (
    <div className="max-w-2xl space-y-6">
      <Link href="/station-management/radio" className="inline-flex items-center gap-1.5 text-xs font-semibold text-muted-foreground hover:text-[#02B2FF] transition-colors">
        <ArrowLeft size={13} /> Back to Radio Stations
      </Link>

      <div>
        <h1 className="text-xl font-bold text-foreground">Add Radio Station</h1>
        <p className="text-sm text-muted-foreground mt-1">Register a new radio station on the platform</p>
      </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
          ...
          <div className="flex items-center gap-3 pt-2">
            <Button type="submit" disabled={isLoading} className="bg-[#02B2FF] hover:bg-[#0190D0] text-white">
              ...
              {isLoading ? "Creating..." : "Create Radio Station"}
            </Button>
